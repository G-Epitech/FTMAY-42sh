...
void display_cursor(input_line_t *line)
{
    ...
    + len_prompt) / (line->buffer->term_size->nb_cols + 1));
    pos_rows_cursor = line->buffer->rows_start_cursor + ((line->buffer->cursor
    + len_prompt + (pos_rows_cursor - line->buffer->rows_start_cursor)) /
    (line->buffer->term_size->nb_cols + 1));
    pos_col_cursor = (line->buffer->cursor + len_prompt + (pos_rows_cursor -
    line->buffer->rows_start_cursor)) % (line->buffer->term_size->nb_cols + 1);
    printf("\x1b[%i;%iH", pos_rows_cursor, pos_col_cursor);
}
